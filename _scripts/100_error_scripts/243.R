...
### Data from https://en.wikipedia.org/wiki/1922_Finnish_parliamentary_election
...
### Suom. Socialistinen työväenpuolue / Finnish Socialist Workers' Party (25)
...
if (anyDuplicated(patch_mags[, c("cst_n")])) {
    stop("Duplicate entries in patches for Finland 1922")
}
...
patch <- patch |>
    dplyr::mutate(pty = case_when(
                      ...
                      Party == "SDP" ~ 24,
                      Party == "Suom. Socialistinen työväenpuolue" ~ 25,
                      ))

if (any(is.na(patch$pty))) {
    stop("Unmapped party in patches for Finland 1922")
}
