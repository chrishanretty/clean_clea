### Finland (October 1930)

### Data from https://en.wikipedia.org/wiki/1930_Finnish_parliamentary_election
...
### SKP / the single seat the file records under "SKP" maps onto the only
###   corresponding CLEA code for 1930, Suomen pienviljelijäin puolue (23).
###   Communist lists were barred from the 1930 election.
...
if (anyDuplicated(patch_mags[, c("cst_n")])) {
    stop("Duplicate entries in patches for Finland 1930")
}
...
if (any(is.na(patch$pty))) {
    stop("Unmapped party in patches for Finland 1930")
}
