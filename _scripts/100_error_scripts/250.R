...
### Data from https://en.wikipedia.org/wiki/1939_Finnish_parliamentary_election
...
### Note: from 1938 the two Oulu constituencies were merged into a single
### "oulun läänin". The file therefore carries the combined total under
### "Oulun l. etel." and marks "Oulun l. pohjoinen" as NA (dropped below).
...
cst_lookup <- c(
    ...
    "Oulun l. etel."       = "oulun läänin",
    "Oulun l. pohjoinen"   = NA_character_,
    "Lapin l."             = "lapin läänin")

patch <- raw |>
    dplyr::filter(Year == 1939) |>
...
### IKL / Isänmaallinen kansanliike (2)
### Kok / Kansallinen Kokoomus (4)
### Edistyspuolue / Kansallinen Edistyspuolue (3)
### Maalaisliitto / Maalaisliitto (8)
### Pienviljelijäin ja maalaistkansan / Suomen pienviljelijäin puolue (23)
...
if (anyDuplicated(patch_mags[, c("cst_n")])) {
    stop("Duplicate entries in patches for Finland 1939")
}
...
patch <- patch |>
    dplyr::mutate(pty = case_when(
                      ...
                      Party == "Maalaisliitto" ~ 8,
                      Party == "Pienviljelijäin ja maalaistkansan" ~ 23,
                      Party == "SDP" ~ 24,
                      ))

if (any(is.na(patch$pty))) {
    stop("Unmapped party in patches for Finland 1939")
}
