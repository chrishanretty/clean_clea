...
### Data from https://en.wikipedia.org/wiki/1933_Finnish_parliamentary_election
...
### Suomen pienvilj p. / Suomen pienviljelijäin puolue (23)
### Kansanpuolue / Kansanpuolue (5)
### Kok / Kansallinen Kokoomus and IKL / Isänmaallinen kansanliike contested
###   1933 on a joint list; CLEA records all of their seats under the joint
###   list code 5002 (c + ppm), so both columns are mapped to 5002 and summed.
...
if (anyDuplicated(patch_mags[, c("cst_n")])) {
    stop("Duplicate entries in patches for Finland 1933")
}
...
patch <- patch |>
    dplyr::mutate(pty = case_when(
                      ...
                      Party == "SDP" ~ 24,
                      Party == "Suomen pienvilj p." ~ 23,
                      Party == "Kansanpuolue" ~ 5,
                      Party == "Kok" ~ 5002,
                      Party == "IKL" ~ 5002,
                      ))

if (any(is.na(patch$pty))) {
    stop("Unmapped party in patches for Finland 1933")
}
