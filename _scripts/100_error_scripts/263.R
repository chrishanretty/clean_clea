...
### Data from https://en.wikipedia.org/wiki/1987_Finnish_parliamentary_election
...
### DV (Demokraattinen Vaihtoehto / Democratic Alternative)
### Vihr (Vihrea liitto / Green League)
### L–S (Aland liberal list)
...
if (anyDuplicated(patch_mags[, c("cst")])) {
    stop("Duplicate entries in patches for Finland 1987")
}
...
parties <- c("SDP", "Kok", "Kesk", "SKDL", "RKP", "SMP", "SKL", "DV", "Vihr", "L–S")
patch <- patch |>
    ...
    mutate(pty = case_when(
                           ...
                           name == "DV" ~ 1,
                           name == "Vihr" ~ 29,
                           name == "L–S" ~ 4000  ## recoded as others
                           )) |>
