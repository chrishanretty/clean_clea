...
### Data from https://en.wikipedia.org/wiki/1966_Finnish_parliamentary_election
...
### SPP (Suomen Pientalonpoikien Puolue / Finnish Small Farmers' Party)
...
if (anyDuplicated(patch_mags[, c("cst")])) {
    stop("Duplicate entries in patches for Finland 1966")
}
...
parties <- c("SDP", "Kesk", "SKDL", "Kok", "RKP", "LKP", "TPSL", "SPP", "ÅS")
patch <- patch |>
    ...
    mutate(pty = case_when(
                           ...
                           name == "LKP" ~ 7,
                           name == "TPSL" ~ 27,
                           name == "SPP" ~ 4000,  ## recoded as others
