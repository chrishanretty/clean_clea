...
### Data from https://en.wikipedia.org/wiki/1924_Finnish_parliamentary_election
...
### Työläisten ja pienviljelijäin p. / Workers' and Smallholders' Party,
###   the communist front list (Suomen Työväenpuolue, 26)
...
if (anyDuplicated(patch_mags[, c("cst_n")])) {
    stop("Duplicate entries in patches for Finland 1924")
}
...
patch <- patch |>
    dplyr::mutate(pty = case_when(
                      ...
                      Party == "SDP" ~ 24,
                      Party == "Työläisten ja pienviljelijäin p." ~ 26,
                      ))

if (any(is.na(patch$pty))) {
    stop("Unmapped party in patches for Finland 1924")
}
