...
### Data from https://en.wikipedia.org/wiki/1927_Finnish_parliamentary_election
...
### Sosialistinen työväen ja pienviljelijäin p. / Socialist Workers' and
###   Smallholders' Party, the communist front list (Suomen Työväenpuolue, 26)
...
if (anyDuplicated(patch_mags[, c("cst_n")])) {
    stop("Duplicate entries in patches for Finland 1927")
}
...
patch <- patch |>
    dplyr::mutate(pty = case_when(
                      ...
                      Party == "SDP" ~ 24,
                      Party == "Sosialistinen työväen ja pienviljelijäin p." ~ 26,
                      ))

if (any(is.na(patch$pty))) {
    stop("Unmapped party in patches for Finland 1927")
}
